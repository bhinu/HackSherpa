          ...
          {[1, 2, 3, 4].map((step) => (
            <div
              ...
              className={cn(
                ...
                currentStep >= step ? "bg-primary text-primary-foreground" : "border bg-background",
              )}
            >
              {step}
            </div>
      ...
      <Form {...form}>
        <form className="space-y-8">
          ...
          {currentStep === 4 && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                {generatedReadme && (
                  <Button type="button" variant="outline" size="icon" onClick={handleCopyReadme}>
                    {isReadmeCopied ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}
                  </Button>
                )}
              </div>
              {generatedReadme ? (
                <pre className="whitespace-pre-wrap rounded-lg border p-4 text-sm">{generatedReadme}</pre>
              ) : (
                <div className="text-center text-muted-foreground">Click Generate to create your README</div>
              )}
            </div>
          )}
          ...
          <div className="flex justify-between pt-8">
            ...
            {currentStep === 4 && (
              <Button type="button" className="ml-auto" onClick={() => handleSubmit(form.getValues())}>
                Generate README
              </Button>
            )}
          </div>
